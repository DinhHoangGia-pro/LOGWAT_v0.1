...
def draw_updated_figure_2():
    ...
    # Minh họa việc kết nối SELECT và FROM bất chấp các token rác ở giữa
    G2 = nx.DiGraph()
    ...
    # Cạnh ngữ nghĩa nối trực tiếp SELECT (idx 2) và FROM (idx 5)
    edges_semantic2 = [(2, 5)] 
    ...
    # Vẽ cạnh Semantic (Màu đỏ, nét đứt, cong lên)
    ...
    # Minh họa việc bỏ qua các comment/noise để nối các phần của payload
    G3 = nx.DiGraph()
    n3_labels = ["<scr", "/**/", "ipt>", "alert", "(", "1", ")"]
    G3.add_nodes_from(range(len(n3_labels)))
    
    edges_seq3 = [(i, i + 1) for i in range(len(n3_labels) - 1)]
    # Skip Edge: Nhảy qua comment /**/ (idx 0 -> 2)
    # Semantic Edge: Nối script tag tới hàm alert (idx 2 -> 3)
    ...
    # Vẽ cạnh đặc biệt (Màu xanh dương, nét đứt, cong xuống)
