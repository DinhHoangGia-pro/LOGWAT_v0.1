...
def build_web_graphs():
    ...
    # Định nghĩa các cặp từ khóa thường đi cùng nhau trong tấn công
    semantic_pairs = [
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Deep Building"):
        ...
        for i in range(num_nodes):
            if i < num_nodes - 1: edges.extend([[i, i+1], [i+1, i]]) # Cạnh kề
            if i < num_nodes - 2: edges.extend([[i, i+2], [i+2, i]]) # Cạnh nhảy
            
            # Thêm cạnh phụ thuộc ngữ nghĩa trong phạm vi 15 tokens
