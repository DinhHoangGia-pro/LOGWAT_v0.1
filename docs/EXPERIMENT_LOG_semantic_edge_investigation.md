...
> **Correction (2026-09-20).** The first half of this note stands (the case variation is removed by `.lower()`); the second half ("same capability as `sql_new_commands`", "not for obfuscation robustness") is withdrawn. The cell's content (`scripts/build_heldout_matrix_eval.py:52`) is `q={i};uN/**/ioN aLl sEl/**/eCt * frOm admins WH/**/ere id={i};--`: it has mixed case AND `/**/` inserted inside three keywords. After lowercasing, the token stream is `un / * * / ion all sel / * * / ect * from admins wh / * * / ere …` (32 tokens), and `semantic_edges()` gives 4 E_sem edges, `(9,16),(16,9),(16,18),(18,16)` = union↔select and select↔from, the same two pairs as the `comment_splitting` cell (`(9,16),(16,9),(16,23),(23,16)`, 37 tokens; verified by running tokenizer + `semantic_edges`). So what remains after the case part is neutralized is a comment-splitting row (different split points, `from` left whole), not plain SQLi. Do NOT write "case_mixing fails identically to comment_splitting under ablation": `comment_splitting` is 0/10 in all six configs including the full model, while `case_mixing` is 10/10 in configs (1)-(2) and in all 5 seeds of the official config and reaches 0/10 only when E_skip or E_sem is removed. What is supported: ablation brings it to the outcome comment_splitting always has; why the full model passes one and not the other is not isolated. The paper (`sec:threat_model`, `sec:ablation` in sn-article_v1.tex) states both facts.
