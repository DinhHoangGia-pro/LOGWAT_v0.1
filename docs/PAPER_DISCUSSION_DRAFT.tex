...
\emph{(a) A real trade-off.} Specializing part of the network for $E_{sem}$ may cost general representational strength in the regime that dominates the external set, very short and unfamiliar graphs that the training data do not cover (see the discussion of HGT's external instability above). One observation favours this: the modified model's SQLi recall (0.884) is below that of both stock LOGWAT (0.948) and the placebo (0.954), and this is where the added path is active, since 49\% of external SQLi requests contain a semantic edge (Welch $p=0.08$ against the placebo, uncorrected and only suggestive at five seeds). Against it, the drops in external Benign recall (0.577) and weighted F1 do not separate from the placebo ($p=0.20$ and $0.13$), and Benign requests never activate the path.
